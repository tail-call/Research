# https://programmersought.com/article/3507546697/
    ...
    def backward(self, Ω):
        ...
        for i in range(self.grad.shape[0]):
            for j in range(self.grad.shape[1]):
                grad_output_tmp[i,j,:] = Ω[int(i/4),int(j/4),:]
...
class Network:
    ...
    def backward(self, X, y, learning_rate):
        ...
        for i in reversed(range(len(self.ℒs))):
            ℒ = self.ℒs[i]
            if i == len(self.ℒs) - 1:
                ℒ.error = error
                ℒ.delta = ℒ.error * ς(output)
            else:
                nℒ = self.ℒs[i + 1]
                ℒ.error = np.dot(
                    nℒ.delta,
                    np.array([neuron.weights for neuron in nℒ.neurons])
                )
                ℒ.delta = ℒ.error * ς(ℒ.output)
        
        for i in range(len(self.ℒs)):
            ℒ = self.ℒs[i]
            # Для первого слоя вход это input, для остальных — выходы
            # предыдущих слоёв...
            inputs = X if i == 0 else self.ℒs[i - 1].output
            for neuron in ℒ.neurons:
                for j in range(len(neuron.weights) - 1):
                    neuron.weights[j] += learning_rate * ℒ.delta[j] * inputs[j]
                neuron.weights[-1] += learning_rate * ℒ.delta[-1]